...
contract Sensor{
    ...
    event newSensorEvent (uint256 _sendorId, string name);

    function setStorageData (string memory _date, string memory _hash, uint256 _sendorId) external  returns (bytes32 ){
    ...
    function getStorageData (string memory _date)external view  returns (bytes32 ){
        return filehash[_date];
    }

    function setSensor(uint256 _sensorId, string memory name)public returns(bool){
