...
contract Lager {
    ...
    function existStorage(uint256 _lagerId) internal view returns (uint256 r) {
        ...
        for (uint256 i = 0; i < bottles.length; i++) {
            for (uint256 j = 0; j < bottles.length; j++) {
                if (
                    Data[_lagerId][i].time == bottles[j].time &&
                    WineNFTs(adminNFTS).hashCompareWithLengthCheck(
                        Data[_lagerId][i].Udi,
                        bottles[j].Udi
                    )
                ) {
    ...
    function checkIn(uint256 _lagerId, string memory _Udi)
        public
        returns (bool r)
    {
        ...
        require(
            ...
            "This bottle is not found for checkin"
        );
        require(
            WineNFTs(adminNFTS).alreadyExist(_Udi) == true,
            "a bottle with this Udi is not aviable"
        );
        r = false;
    ...
    function checkOut(uint256 _lagerId, string memory _Udi)
        public
        returns (bool r)
    {
        require(
            setterLager[_lagerId] == true,
            "we don't have a lager with this ID"
        );
        require(
            setterBottle[_Udi] == true,
            "This bottle is not found for checkout"
        );
        r = false;
        uint256 actuel;
        uint256 helper = 0;
    ...
    function newLager(uint256 _lagerId, string memory location)
        public
        returns (bool)
    {
        require(
            setterLager[_lagerId] == false,
            "we already have a lager with this ID"
        );
    ...
    function newSensor(
        ...
        require(
            setterLager[_lagerId] == true,
            "we don't have a lager with this name"
        );
    ...
    function get_sendor_data(string memory _date)
        external
        view
        returns (bytes32 r)
    {
